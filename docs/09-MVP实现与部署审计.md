...
当前版本已经形成“上传论文 → 异步解析 → 引用规范化与开放全文获取 → 任务级临时索引 → 双文档文本证据 → 人工裁决 → 报告 → 临时材料清理”的可运行切片。它仍不是全部科研诚信检测能力：语义改写、引用支持关系、完整统计复算和高级图片取证尚未实现，页面与报告必须继续显示实际覆盖范围。
...
1. 当前文本来源检查以连续词面重合为主，尚未接入 BGE-M3/FAISS 语义召回与翻译式改写检索。
2. 引用论断支持、撤稿更正核验仍未进入统一证据闭环。
3. 统计复算与高级图片取证仍需后续按同一 `EvidenceItem` 契约接入。
