...
SBDC 的正式英文名是 **Source-Based Deep Check**，即“基于来源与证据的深度核查”。它是一个面向博士论文和期刊论文的科研诚信深度检查项目，围绕一篇待检论文即时获取其引用文献，构建任务级临时对照库，发现文本复用、引用失真、统计与数据异常、图片复用及跨版本不一致，并把每一项发现还原为可复核证据。
...
- 当前交付：审查者登录、浏览器上传、异步解析、引用规范化、合法开放全文获取、任务级临时词面索引、双文档文本证据、人工裁决、版本化报告与可审计临时资产清理。
- 已确认边界：不建设长期全文对照库；每次任务只为当前论文临时获取和索引引用文献。
- 当前深度审核已在同一证据契约内接入任务级语义近似候选、引用论断支持核对、可复算统计表达式和 PDF 图片局部区域筛查，并在工作台与报告中披露逐项覆盖率和计算上限。后续增强仍聚焦跨语言/嵌入式语义召回、更多统计格式、表格数据以及原始图片取证。
