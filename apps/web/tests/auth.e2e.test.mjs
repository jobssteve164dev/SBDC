...
test("审查者能从证据卡直接定位原文并作出复核决定", async (t) => {
  ...
  try {
    ...
    assert.match(await page.locator("main").innerText(), /词面比较 300\/50000/);
    assert.match(await page.locator("main").innerText(), /语义近似[\s\S]*3/);
    assert.match(await page.locator("main").innerText(), /引用论断核对[\s\S]*5\/18[\s\S]*50000\/50000/);
    assert.match(await page.locator("main").innerText(), /统计复算[\s\S]*2\/34/);
    assert.match(await page.locator("main").innerText(), /阈值 8\/1000；均值 4\/500；列表比较 12\/5000/);
    assert.match(await page.locator("main").innerText(), /图片局部核对[\s\S]*260\/50000[\s\S]*跳过 1；稀疏采样 2；像素 128000\/40000000；图块 2500\/25000/);
    assert.match(await page.locator("main").innerText(), /部分分析达到计算上限/);
    ...
    await page.getByRole("heading", { name: "不同条件的结果来自不同研究对象" }).waitFor();
    await page.getByRole("heading", { name: "引用论断与来源方向需要核对" }).waitFor();
    await page.getByRole("heading", { name: "报告平均值与所列数值不一致" }).waitFor();
    assert.doesNotMatch(await page.locator("main").innerText(), /citation_direction_conflict_candidate|statistical_average_inconsistency/);
    ...
    assert.equal(await page.getByLabel("复核决定").first().isVisible(), true);
    assert.equal(await page.getByLabel("裁决理由").first().isVisible(), true);
